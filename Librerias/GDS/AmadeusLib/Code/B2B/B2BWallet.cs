 ...
 
        {
            ...
            try
            {
                using (var lservice = Configuracion.GetServiceModelClient<IB2BWallet_1AWSChannel>())
                {
                    ...
                    using (var lwebServiceFileValueAmadeus = new B2BServiceFileValueAmadeus<TIn>(operation))
                    {
                    ...
                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Por ejecutar 'IB2BWallet_1AWSChannel.ProcessRequest'", new { lrequestXml }, CodigoSeguimiento);

                    // ejecutando servicio y leyendo string xml del response
                    lresponseXml = lservice.ProcessRequest(lrequestXml);

                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Ejecutado 'IB2BWallet_1AWSChannel.ProcessRequest'", new { lresponseXml }, CodigoSeguimiento);
            ...
            {
                // registrando eventos
                Bitacora.Current.ErrorAndInfo("ERROR ejecutando 'IB2BWallet_1AWSChannel.ProcessRequest'", ex, new { MuteErrors, parameters, result, lrequestXml, lresponseXml }, CodigoSeguimiento);
