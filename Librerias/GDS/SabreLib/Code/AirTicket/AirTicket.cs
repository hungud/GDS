    ...
    public sealed class AirTicket : Common<Security1, MessageHeader, SecurityUsernameToken, From, To, PartyId, Service, MessageData>
    {
                                   ...
                                   string numeroPQ)
        {
            ...
            try
            {
                ...
                {
                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Por ejecutar 'AirTicketPortTypeChannel.AirTicketRQ'", new { lairTicketRQRequest }, CodigoSeguimiento);

                    // procesando solicitud
                    lairTicketRQResponse = lservicio.AirTicketRQ(lairTicketRQRequest);

                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Ejecutado 'AirTicketPortTypeChannel.AirTicketRQ'", new { lairTicketRQResponse }, CodigoSeguimiento);
            ...
            {
                // registrando eventos
                Bitacora.Current.ErrorAndInfo(ex, new { MuteErrors, esReemision, numeroPQ, lairTicketRQRequest, lairTicketRQResponse, lrespuesta }, CodigoSeguimiento);
