...
namespace SabreLib.Herramientas
{
    public sealed class SessionCreate : Common<Security, MessageHeader, SecurityUsernameToken, From, To, PartyId, Service, MessageData>
    {
        ...
        /// <param name="sesion"></param>
        /// <returns></returns>
        public CE_Estatus Execute(ref CE_Session sesion)
        {
            ...
            try
            {
                ...
                {
                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Por ejecutar 'SessionCreatePortTypeChannel.SessionCreateRQ'", new { lsessionCreateRQRequest }, CodigoSeguimiento);

                    // procesando solicitud
                    lsessionCreateRQResponse = lservicio.SessionCreateRQ(lsessionCreateRQRequest);

                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Ejecutado 'SessionCreatePortTypeChannel.SessionCreateRQ'", new { lsessionCreateRQResponse }, CodigoSeguimiento);
            ...
            {
                // registrando eventos
                Bitacora.Current.ErrorAndInfo(ex, new { MuteErrors, lsessionCreateRQRequest, lsessionCreateRQResponse, lrespuesta }, CodigoSeguimiento);
