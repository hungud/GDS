    ...
    public sealed class EndTransaction : Common<Security1, MessageHeader, SecurityUsernameToken, From, To, PartyId, Service, MessageData>
    {
        ...
        private CE_Estatus Execute(bool cerrar)
        {
            ...
            try
            {
                ...
                {
                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Por ejecutar 'EndTransactionPortTypeChannel.EndTransactionRQ'", new { lendTransactionRQRequest }, CodigoSeguimiento);

                    // procesando solicitud
                    lendTransactionRQResponse = lservicio.EndTransactionRQ(lendTransactionRQRequest);

                    // registrando eventos
                    Bitacora.Current.DebugAndInfo("Ejecutado 'EndTransactionPortTypeChannel.EndTransactionRQ'", new { lendTransactionRQResponse }, CodigoSeguimiento);
            ...
            {
                // registrando eventos
                Bitacora.Current.ErrorAndInfo(ex, new { MuteErrors, lendTransactionRQRequest, lendTransactionRQResponse, lrespuesta }, CodigoSeguimiento);
