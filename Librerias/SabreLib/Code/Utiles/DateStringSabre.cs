...
{
    internal static class DateStringSabre
    {
        // =============================
        // metodos estaticos
                                            ...
                                            bool isForItinerary)
        {
            ...
            if (!string.IsNullOrWhiteSpace(lvalue))
            {
                ...
                // evaluando si se pudo hallar un valor de fecha (yyyy-mm-dd)
                if (Regex.Match(lvalue, @"^\d{4}-\d{2}-\d{2}$").Success)
                {
                    // diviendo valor de fecha
                    lvalores = lvalue.Split('-');

                    // construyendo un objeto de fecha
                    lfecha = new DateTime(int.Parse(lvalores[0]), int.Parse(lvalores[1]), int.Parse(lvalores[2]), 0, 0, 0);
                }

                // evaluando si se pudo hallar un valor de fecha (mm-dd)
                if (Regex.Match(lvalue, @"^\d{2}-\d{2}$").Success)
                {
                    // diviendo valor de fecha
                    lvalores = lvalue.Split('-');

                    // construyendo un objeto fecha
                    var lfechaTemporal = new DateTime(lfecha.Value.Year, int.Parse(lvalores[0]), int.Parse(lvalores[1]), 0, 0, 0);
                    ...
                    // construyendo un objeto de fecha
                    lfecha = new DateTime(lfechaTemporal.Year, int.Parse(lvalores[0]), int.Parse(lvalores[1]), 0, 0, 0);
                }
        ...
        public static DateTime? ToValidBirthdate(string value)
        {
            ...
            if (!string.IsNullOrWhiteSpace(lvalue))
            {
                ...
                // buscando fecha con formato "yyyy-mm-dd" o "ddMMMYYYY"
                var lencontrado = Regex.Match(lvalue, @"^(\d{2})(\w{3})(\d{4})|(\d{4})-(\d{2})-(\d{2})$");

                // evaluando si se pudo hallar un valor de fecha "yyyy-mm-dd" o "ddMMMYYYY"
                if (lencontrado.Success)
                {
                    ...
                    // construyendo un objeto de fecha
                    lfecha = new DateTime(
                        ((lvalue.IndexOf("-", StringComparison.InvariantCultureIgnoreCase) == -1) ? int.Parse(lencontrado.Groups[3].Value) : int.Parse(lencontrado.Groups[1].Value)),
                        lmes, 
                        int.Parse(lencontrado.Groups[1].Value), 
                        0, 
                        0, 
                        0);
                ...
                {
                    // buscando fecha con formato "ddMMMYY"
                    lencontrado = Regex.Match(lvalue, @"^(\d{2})(\w{3})(\d{2})$");
                    ...
                    if (lencontrado.Success)
                    {
                        ...
                        var ldiferencia = (int.Parse(lfecha.Value.Year.ToString().Substring(2, 2)) - int.Parse(lencontrado.Groups[3].Value));

                        // construyendo un objeto de fecha
                        lfecha = new DateTime(
                            int.Parse(((ldiferencia < 0) ? "19" : "20") + lencontrado.Groups[3].Value),
                            lmes, 
                            int.Parse(lencontrado.Groups[1].Value), 
                            0, 
                            0, 
