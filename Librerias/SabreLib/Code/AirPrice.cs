    ...
    public sealed class AirPrice : Common<Security1, MessageHeader, SecurityUsernameToken, From, To, PartyId, Service, MessageData>
    {
                                   ...
                                   out CE_Cotizacion cotizacion)
        {
            ...
            if (response.ApplicationResults.status == CompletionCodes.Complete)
            {
                ...
                {
                    Pseudo = response.PriceQuote.PricedItinerary.AirItineraryPricingInfo[0].FareCalculationBreakdown[0].Branch.PCC,
                                  ...
                                  out CE_Cotizacion cotizacion)
        {
            ...
            try
            {
                ...
                {
                    // registrando eventos
                    Bitacora.Current.InfoAnd(PartnerLevel.Debug, "Por ejecutar 'OTA_AirPricePortTypeChannel.OTA_AirPriceRQ'", null, new { lotaAirPriceRQRequest }, CodigoSeguimiento);

                    // procesando solicitud
                    lotaAirPriceRQResponse = lservicio.OTA_AirPriceRQ(lotaAirPriceRQRequest);

                    // registrando eventos
                    Bitacora.Current.InfoAnd(PartnerLevel.Debug, "Ejecutado 'OTA_AirPricePortTypeChannel.OTA_AirPriceRQ'", null, new { lotaAirPriceRQResponse }, CodigoSeguimiento);
            ...
            {
                // registrando eventos
                Bitacora.Current.InfoAnd(PartnerLevel.Warn, "ERROR ejecutando '.Execute'", ex, new { MuteErrors, parametros, cotizacion, lotaAirPriceRQRequest, lotaAirPriceRQResponse, lrespuesta }, CodigoSeguimiento);
            ...
            {
                // registrando eventos
                Bitacora.Current.InfoAnd(PartnerLevel.Error, "ERROR ejecutando '.Execute'", ex, new { MuteErrors, parametros, cotizacion, lotaAirPriceRQRequest, lotaAirPriceRQResponse, lrespuesta }, CodigoSeguimiento);
